...
import 'package:testing/DoctorPatient.dart';
...
class _SearchPatientState extends State<SearchPatient> {
  ...
  startSearch() {
    databaseMethods.getUserByUsername(searchForController.text).then((val) {
      setState(() {
        searchSnapshot = val;
      });
    });
  ...
  createChat(String user1) {
    if (user1 != Constants.myName) {
      ...
      databaseMethods.createProfileRoom(chatRoomId, chatRoomMap);
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => DoctorPatient(user1)));
    } else {
...
getChatRoomId(String first, String second) {
  if (first.substring(0, 1).codeUnitAt(0) >
      second.substring(0, 1).codeUnitAt(0)) {
